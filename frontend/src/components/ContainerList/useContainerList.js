...
export function useContainerList() {
    ...
    async (showRefresh = false) => {
      ...
      try {
        ...
        if (response.data.success) {
          const validStacks = response.data.stacks.filter(
            (stack) =>
              stack.id != null && stack.id !== undefined && stack.id !== "",
          );
          setStacks(validStacks);
          setLastUpdate(new Date());
          console.log(
            `Загружено ${validStacks.length} стеков из ${response.data.stacks.length} за ${response.data.duration_ms || performance.now() - startTime}мс`,
          );
          console.log(
            `Отфильтровано ${response.data.stacks.length - validStacks.length} стеков без ID`,
          );
        } else {
  ...
  const checkStackHealth = useCallback(
    async (stackId, stackName) => {
      if (!stackId) {
      ...
      try {
        const response = await axios.post(
          `${API_URL}/sandboxes/${stackId}/check-health`,
        );
  ...
  const deleteStack = useCallback(async () => {
    const stackName = deleteDialog.stackName;
    const stackId = deleteDialog.stackId;
    ...
    API_URL,
    deleteDialog.stackName,
    deleteDialog.stackId,
    fetchDashboardData,
  ]);
