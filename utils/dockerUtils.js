...
function pauseBackgroundUpdate() {
    ...
    console.log(`Фоновое обновление приостановлено (pauseCount: ${backgroundPauseCount})`);
}

function resumeBackgroundUpdate() {
    if (backgroundPauseCount > 0) {
        backgroundPauseCount--;
        console.log(`Фоновое обновление возобновлено (pauseCount: ${backgroundPauseCount})`);
    }
}
...
function getStacks() {
    const containersOutput = execSync('docker ps -a --format "{{.Names}}"', { encoding: 'utf8' });
    const allContainers = containersOutput.trim().split('\n').filter(n => n);
...
function getContainerPorts(stackName) {
    try {
        const containers = execSync(`docker ps --filter "name=${stackName}" --format "{{.Names}}|{{.Ports}}"`, { encoding: 'utf8' });
        ...
        lines.forEach(line => {
            ...
            if (name.includes('_frontend')) {
                const match = portsStr.match(/0\.0\.0\.0:(\d+)->3000/);
...
function getDetailedContainerInfo(stackName) {
    try {
        const containersOutput = execSync(`docker ps -a --filter "name=${stackName}" --format "{{json .}}"`, { encoding: 'utf8' });
        ...
        console.error(`Ошибка получения информации о контейнерах для ${stackName}:`, err.message);
        return [];
    }
}
...
async function startStackAsync(git_branch, stackType, stackName, operationId) {
    ...
    try {
        ...
        await execPromise(`git clone --branch ${git_branch} --depth 1 https://github.com/XtadeRe/ibank-project.git "${stackDir}"`);
        ...
        const envContent = `STACK_NAME=${stackName}
...
`;
                ...
                function updateHtmlFiles(dir) {
                    ...
                    files.forEach(file => {
                        ...
                        if (stat.isDirectory()) {
                            ...
                            html = html.replace(/localhost:\{\{APP_PORT\}\}/g, `localhost:${containerPorts.app}`);
        ...
        if (stackType === 'full') {
            ...
            try {
                 await execPromise(`docker exec ${stackName}_php php artisan key:generate --force`);
                await execPromise(`docker exec ${stackName}_php php artisan migrate --force`);
                await execPromise(`docker exec ${stackName}_php php artisan config:clear`);
        ...
        const result = {
            ...
            urls: {
                web: finalContainerPorts.web ? `http://localhost:${finalContainerPorts.web}` : null,
                api: finalContainerPorts.web ? `http://localhost:${finalContainerPorts.web}/api` : null,
                app: finalContainerPorts.app ? `http://localhost:${finalContainerPorts.app}` : null,
                appApi: finalContainerPorts.app ? `http://localhost:${finalContainerPorts.app}/api` : null,
                frontend: finalContainerPorts.frontend ? `http://localhost:${finalContainerPorts.frontend}` : null,
                phpmyadmin: finalContainerPorts.phpmyadmin ? `http://localhost:${finalContainerPorts.phpmyadmin}` : null
            },
            portsInfo: {
        ...
        console.log(`Результат стека "${stackName}":`, JSON.stringify(result, null, 2));
...
async function deleteStackAsync(stackName, operationId) {
    ...
    try {
        ...
        const containers = execSync(`docker ps -a --filter "name=${stackName}" --format "{{.ID}}"`, { encoding: 'utf8' });
...
async function waitForContainer(containerName, maxAttempts = 30) {
    for (let i = 1; i <= maxAttempts; i++) {
        ...
        if (i === maxAttempts) {
            console.log(`Контейнер ${containerName} не запустился после ${maxAttempts} попыток`);
            return false;
        }
